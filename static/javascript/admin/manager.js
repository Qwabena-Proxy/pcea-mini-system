...
const saveAcademicYearHandler = (e) => {
    ...
    const levelTutions= document.querySelectorAll(".tution > input[type='text']")
    let formData= new FormData();
    ...
    formData.append("academic_year_levels", Array.from(document.querySelectorAll("#level-data option")).map(option => option.value).join(","));
    formData.append("academic_year_levels_tution", Array.from(levelTutions).map(input => input.value).join(","));
    ...
    console.log(academicYear.value, academicYearStart.value, academicYearEnd.value)
